...
export class OpenAIResponseHandler {
  private message_text = ""; // simple text
  private chunk_counter = 0; // AI should print the data in chunks
  private run_id = ""; // Each request when run it has a unique id
  ...
  private handleStopGenerating = async (event: Event) => {
    ...
    if (!this.openAiThread || !this.openai || !this.run_id) {
      return;
    }
    try {
  ...
  private performWebSearch = async (query: string): Promise<string> => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.log(`Travily search failed for query: ${query} `, errorText);
      ...
      console.log(`Travily Search Successfull for query: ${query}`);
